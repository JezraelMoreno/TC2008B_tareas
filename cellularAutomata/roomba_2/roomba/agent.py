...
class TaskType:
    ...
    # Etiquetas simples para los tipos de tarea
    CLEANING = "cleaning"
    RETURNING = "returning"
...
class ScanState(RoombaState):
    ...
    def step(self):
        ...
        # Revisar el estado del cuarto antes de actuar
        ...
        # No hay celdas sucias -> recargar o esperar en una estación
        if agent.at_any_station:
...
class NavigateState(RoombaState):
    ...
    def step(self):
        ...
        # Moverse con heurística Manhattan respetando las reservas
...
class CleanState(RoombaState):
    ...
    def on_enter(self):
        # Liberar la celda objetivo para que otros agentes puedan seleccionarla
...
class RechargeState(RoombaState):
    ...
    def step(self):
        ...
        if not agent.at_any_station:
            agent.change_state(ScanState)
            return

        # Aumentar la batería mientras se está en cualquier estación
        agent.recharge()
        if agent.battery >= agent.max_battery:
...
class IdleState(RoombaState):
    ...
    def step(self):
        ...
        if agent.at_any_station and agent.battery < agent.max_battery:
            # Seguir cargando aunque no haya nuevas tareas
            agent.recharge()
...
class RoombaAgent(CellAgent):
    ...
    def __init__(
        ...
        # Contexto dinámico del agente dentro de la máquina de estados
        self.intent = None
        self.target = None
        self.known_dirty = []

        # Caché de estados para reutilizar instancias entre pasos
        self._cached_states = {}
    ...
    def _move_towards(self, target):
        ...
        if best_cells:
            ...
            self.cell = chosen
            self.model.moves[self.agent_id] += 1
            self._drain()
            return True
        return False

    def _drain(self):
        """restar una unidad de batería por acción."""
        if self.battery > 0:
    ...
    @property
    def at_any_station(self) -> bool:
        return any(isinstance(agent, ChargingStation) for agent in self.cell.agents)

    def needs_recharge(self) -> bool:
        """Decide si el agente debe regresar a la estación."""
        # Priorizar regresar cuando la batería está baja
        return (self.battery <= self.return_threshold and not self.at_any_station) or self.battery <= 0

    def clear_target(self):
        """Liberar la reserva del objetivo actual."""
        # Cada agente libera su objetivo para evitar conflictos
    ...
    def scan_environment(self):
        ...
        # Guardar el resultado para usarlo en los siguientes pasos
    ...
    def select_dirty_target(self, dirty_cells):
        ...
        for coordinate in ordered:
            # Seleccionar la celda sucia disponible más cercana
    ...
    def clean_current_cell(self) -> bool:
        ...
        if dirt:
            dirt[0].clean()
            self.model.cleaned_cells[self.agent_id] += 1
            self._drain()
            return True
        return False
